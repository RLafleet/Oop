...
bool Translate(const std::string& pairOriginalWords, const std::string& pairTranslateWords, const std::string& word)
{
	std::istringstream iss(pairOriginalWords);
	std::string token;
	...
	while (std::getline(iss, token, ','))
	{
		...
		{
			std::cout << pairOriginalWords << " translates as " << pairTranslateWords << std::endl;
			return true;
		}
	}
	return false;
...
{
	std::cout << "Unknown word " << word << ". Enter translate of that word or do enter without symbols for reject saving." << std::endl;
	std::string input;
...
void SaveDictionary(const std::multimap<std::string, std::string>& dictionary, const std::string& dictionaryFileName)
{
	...
	do
	{
		std::cout << "If you want to save your dictionary put Y/y.\nIf you don't want this put N/n" << std::endl;
		if (!(std::cin >> optionChar))
		{
			throw std::runtime_error("Invalid character");
		}
	}
	while (std::tolower(optionChar) != 'n' && std::tolower(optionChar) != 'y');
	
	if (std::tolower(optionChar) == 'y')
	{
		for (const auto& pair : dictionary)
		{
	...
	std::cout << "Dictionary not saved" << std::endl;
}