...
CDate::CDate(unsigned int day, Month month, unsigned year)
{
    ...
    // цикл убрать.
    while (yearCircle > MIN_AVAILIBALE_YEAR)
    {
        m_days += DAYS_IN_YEAR;
...
unsigned CDate::GetDay() const
{
    ...
    days -= static_cast<int>((year - MIN_AVAILIBALE_YEAR) * DAYS_IN_YEAR 
    ...
    for (unsigned index = 0; index < DAYS_IN_MONTH.size(); index++)
    {
        ...
        bool isFebruary = indexByMonth == static_cast<int>(Month::FEBRUARY);
        bool exceedsDaysInFebraury = (indexByMonth == static_cast<int>(Month::FEBRUARY)
            && days > daysInMonth + IsLeapYear(year));
        bool exceedsDaysInOtherMonths = (indexByMonth != static_cast<int>(Month::FEBRUARY)
            && days > daysInMonth);

        if (exceedsDaysInFebraury || exceedsDaysInOtherMonths)
        {
            days -= daysInMonth;
            if (isFebruary)
            {
...
Month CDate::GetMonth() const
{
    ...
    for (unsigned index = 0; index < DAYS_IN_MONTH.size(); index++)
    {
        ...
        bool isFebruary = indexByMonth == static_cast<int>(Month::FEBRUARY);
        bool exceedsDaysInFebraury = (indexByMonth == static_cast<int>(Month::FEBRUARY)
            && days > daysOfMonth + IsLeapYear(year));
        bool exceedsDaysInOtherMonths = (indexByMonth != static_cast<int>(Month::FEBRUARY)
            && days > daysOfMonth);
